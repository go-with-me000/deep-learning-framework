...
export default {
  ...
  methods: {
    ...
    deleteControls(index) {
      ...
      let URL = `${apiPath}/collegeAdmin/deleteCollegeAdmin`;
    ...
    addControls() {
      let URL = `${apiPath}/collegeAdmin/addCollegeAdmin`;
      axios({
        url: URL,
        method: "get",
        headers: {
          operatedId: this.forms.userId
        }
      })
        .then(res => {
          if (res.data.code === "SUCCESS") {
    ...
    loadData() {
      let URL = `${apiPath}/collegeAdmin/getCollegeAdmin`;
